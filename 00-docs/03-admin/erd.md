...
The model is centered on three actors: Platform Admin, Channel User, and ChannelEndUser.  
모델은 Platform Admin, Channel User, ChannelEndUser의 세 가지 존재를 중심으로 구성됩니다.
...
erDiagram
    ...
    T_CHANNEL ||--o{ T_CHANNEL_END_USER: owns
    T_CHANNEL ||--o{ T_CONVERSATION: contains
    T_CHANNEL_END_USER ||--o{ T_CONVERSATION: starts
...
- `T_CHANNEL_END_USER` is separated from `T_USER` and belongs to a channel.  
  `T_CHANNEL_END_USER`는 `T_USER`와 분리되어 있으며 채널에 속합니다.
...
- Ticket is excluded from the current ERD and treated as a future plan.  
  Ticket은 현재 ERD 범위에서는 제외되며 미래 계획으로 다룹니다.
