...
**목적**: WebSocket 메시지 처리 시 로그에 websocket_command, websocket_session_id, websocket_destination, user_id, auth_session_id를 포함하여 Loki 등에서 필터·집계·추적 가능하게 함.
...
- afterSendCompletion: MdcKeys.WEBSOCKET_COMMAND, WEBSOCKET_SESSION_ID, WEBSOCKET_DESTINATION, USER_ID, AUTH_SESSION_ID 제거 (trace scope 정리와 함께)
- MdcKeys에 WEBSOCKET_COMMAND, WEBSOCKET_SESSION_ID, WEBSOCKET_DESTINATION 추가

**MDC 필드명**: `websocket_command`, `websocket_session_id`, `websocket_destination`, `user_id`, `auth_session_id`

**적용 완료**: WebSocketTracingChannelInterceptor 확장, MdcKeys에 WEBSOCKET_COMMAND, WEBSOCKET_SESSION_ID, WEBSOCKET_DESTINATION 추가