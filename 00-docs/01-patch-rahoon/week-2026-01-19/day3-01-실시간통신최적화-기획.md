...
- **WebSocket**: 
  - 300개 연결까지 **안정적** (에러율 0%)
  - P50 레이턴시: 91~257ms (네트워크 지연 50ms 포함)
  - 연결 수 증가해도 **성능 일정** 유지
  
- **SSE**:
  - 200개까지는 비교적 안정 (에러율 1~3%, 주로 메시지 미수신)
  - **250개부터 급격한 성능 저하** (79.6% 에러율, HTTP 502)
  - **원인 분석**:
    
    **1. HTTP 502 에러 (250개 이상)**:
    - **Tomcat 스레드 풀 고갈** (기본 max-threads: 200개)
    - SSE는 **long-lived HTTP connection** → 연결이 끊어질 때까지 스레드 점유
    - 250개 SSE 연결 = 250개 HTTP 스레드 필요 → 200개 초과 → Nginx "no live upstreams" → 502
    - WebSocket은 Handshake 후 **NIO 기반 비동기 처리** (별도 스레드 풀, HTTP 스레드 즉시 반환)
    - 테스트에서 10개씩 배치 처리하지만, 연결은 계속 유지되므로 누적됨
    
    **2. 메시지 미수신 (50~200개, 1~3%)**:
...
**결론**: 
- **WebSocket**: 높은 동시 연결 수에서도 **안정적** → 최종 후보 1순위
- **SSE**: 200개 이하에서만 안정, 그 이상에서는 **불안정** → 소규모 서비스에만 권장
- 대규모 동시 접속이 예상되는 경우 **WebSocket 선택 권장**
