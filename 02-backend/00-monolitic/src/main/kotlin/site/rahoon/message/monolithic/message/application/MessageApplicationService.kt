...
import site.rahoon.message.monolithic.common.application.CommonResult
import site.rahoon.message.monolithic.common.domain.DomainException
import site.rahoon.message.monolithic.common.global.Base62Encoding
import site.rahoon.message.monolithic.common.global.toEpochMicroLong
import site.rahoon.message.monolithic.common.global.toLocalDateTimeFromMicros
...
class MessageApplicationService(
    ...
    fun create(criteria: MessageCriteria.Create): Message {
        ...
        // 채팅방 멤버인지 확인
        val isMember =
            chatRoomMemberApplicationService.isMember(
        ...
        applicationEventPublisher.publishEvent(MessageEvent.Created.from(message))
        return message
    }
     ...
     * 메시지 조회
     */
    fun getById(messageId: String): Message = messageDomainService.getById(messageId)
    ...
    fun getByChatRoomId(criteria: MessageCriteria.GetByChatRoomId): CommonResult.Page<Message> {
        ...
        val fetched =
            messageDomainService.getByChatRoomId(
                chatRoomId = criteria.chatRoomId,
