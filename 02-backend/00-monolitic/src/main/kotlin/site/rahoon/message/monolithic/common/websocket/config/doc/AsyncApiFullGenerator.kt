...
@Component
class AsyncApiFullGenerator(private val objectMapper: ObjectMapper) {
    ...
    fun generate(basePackage: String): String {
        // Reflections 설정 강화 (스캔 안되는 문제 해결)
...
//            reflections.getMethodsAnnotatedWith(SendToUser::class.java)
        ...
        val doc = mapOf(
            ...
            "channels" to metadataList.associate { meta ->
                val channelId = sanitize(meta.address)

                val channelContent = mutableMapOf( // 주소 자체는 원본 유지
            ...
            "operations" to metadataList.associate { meta ->
                val channelId = sanitize(meta.address)
            ...
            "components" to mapOf(
                ...
                "messages" to metadataList.associate { meta ->
    ...
    // --- 나머지 헬퍼 함수 (기존과 동일) ---
    private fun collectDomainClasses(type: Type, accumulator: MutableSet<Class<*>>) {
        when (type) {
            is Class<*> -> {
                if (type.isArray) collectDomainClasses(type.componentType, accumulator)
    ...
    private fun unwrapType(type: Type): Class<*> = when (type) {
     ...
     */
    private fun simplifyPackageName(canonicalName: String, basePackage: String): String {
     ...
     */
    private fun getTypeName(type: Type, basePackage: String): String = when (type) {
        is Class<*> -> {
            if (type.isArray) {
     ...
     */
    private fun resolveType(type: Type, typeVariableMap: Map<String, Type>): Type {
        return when (type) {
    ...
    private fun createTypeVariableMap(parameterizedType: ParameterizedType): Map<String, Type> {
        ...
        return typeParameters.mapIndexedNotNull { index, typeParam ->
     ...
     */
    private fun generateSchemaForType(type: Type): Map<String, Any> {
        return generateSchemaForType(type, createSchemaGenerator())
    }
     ...
     */
    private fun generateSchemaForType(type: Type, generator: SchemaGenerator): Map<String, Any> {
        return when (type) {
            is Class<*> -> {
                if (type.isArray) {
        ...
        if (properties != null) {
            ...
            properties.forEach { (propertyName, originalFieldSchema) ->
                ...
                if (originalSchema != null) {
                    // 필드 찾기 (정확한 이름 매칭 또는 Jackson이 변환한 이름)
                    val field = fieldMap[propertyKey]
                        ?: fieldMap.values.firstOrNull {
                            // Jackson이 변환한 이름과 매칭 시도
                    ...
                    if (field != null) {
                        ...
                        // 원본 스키마의 다른 속성들(description 등)을 유지하면서 타입 정보 업데이트
                        val mergedSchema = (originalSchema.toMutableMap() as MutableMap<String, Any>).apply {
                            putAll(resolvedSchema)
                        }
                        resolvedProperties[propertyKey] = mergedSchema
                    } else {
                        // 필드를 찾지 못한 경우 원본 스키마 유지
                        resolvedProperties[propertyKey] = originalSchema
                    }
                } else {
    ...
    private fun mapToSchemas(classes: Set<Class<*>>, basePackage: String): Map<String, Any> {
        val config = SchemaGeneratorConfigBuilder(SchemaVersion.DRAFT_2019_09, OptionPreset.PLAIN_JSON)
            .with(JacksonModule(JacksonOption.RESPECT_JSONPROPERTY_ORDER))
            .with(Option.DEFINITIONS_FOR_ALL_OBJECTS)
