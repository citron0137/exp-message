...
class ChatRoomMemberDomainService(
    ...
    fun join(command: ChatRoomMemberCommand.Join): ChatRoomMemberInfo.Detail {
        ...
        // 이미 참가한 멤버인지 확인
        chatRoomMemberRepository.findByChatRoomIdAndUserId(command.chatRoomId, command.userId)
            ?.let {
                throw DomainException(
        ...
        // ChatRoomMember 생성
        val chatRoomMember = ChatRoomMember.create(
    ...
    fun leave(command: ChatRoomMemberCommand.Leave): ChatRoomMemberInfo.Detail {
        ...
        // 멤버인지 확인
        val chatRoomMember = chatRoomMemberRepository.findByChatRoomIdAndUserId(
            command.chatRoomId,
            command.userId
        ) ?: throw DomainException(
    ...
    fun isMember(chatRoomId: String, userId: String): Boolean {
        return chatRoomMemberRepository.findByChatRoomIdAndUserId(chatRoomId, userId) != null
    }
}
